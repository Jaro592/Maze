...
namespace Model
{
    public class RecursivePathFinder : IPathFinder
    {
        ...
        public void FindPath(Maze maze, int[] pos, Queue<int[]> visitedPositions) // akif
        {
            ...
            Solve(maze, pos[0], pos[1], visisted, visitedPositions); // enters the recursion starting from the first coordinate

        }

        private bool Solve(Maze maze, int row, int col, bool[,] visited, Queue<int[]> visitedPositions) // akif
        {
            ...
            visitedPositions.Enqueue(new int[] { row, col });
            ...
            foreach (var move in maze.moves) // call solve for every possible move
            {
                ...
                if (Solve(maze, newRow, newCol, visited, visitedPositions)) // if any of the called solve's return true return true
                    return true;
            }
